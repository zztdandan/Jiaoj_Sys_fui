    ...
    <el-col :span="24" class="buttom-5">
      ...
      <el-form label-width="150px">
        ...
        <el-form-item v-for="item in on_node_detail.act_group" :key="item.name">
          <a :class="item.class" @click="veri_action(item.label,item.condition)">{{item.label}}</a>
        </el-form-item>
      </el-form>
    </el-col>
  </el-row>
...
export default {
  ...
  methods: {
    veri_action(msg,condi) {
      ...
      formdata.append('veri_result', msg);
      formdata.append('condition', condi);
      vue_that
        .$ajax({
          method: 'post',
        ...
        .then(function(res) {
          if (res.data && res.data.flag) {
            vue_that.$alert('审核成功，点击按钮返回审核列表页面', '审核成功', {
              confirmButtonText: '确定',
              callback: action => {
                vue_that.$router.push({ name: 'admin_verify' });
              }
            });
          } else {
            vue_that.$alert(res, '审核失败', {
              confirmButtonText: '确定',
              callback: action => {}
            });
          }
