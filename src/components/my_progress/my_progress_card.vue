  ...
  <div>
    <card-common :title="this.progress_info.PROGRESS_NAME" :has_foot="hasfoot">
      ...
      <template slot="card_foot">
        <a v-if="curr_can_act" @click="do_ondo_click" class="button button-pill button-primary button-small">进行下一步</a>
        <a @click="check_casereview_click" class="button button-pill color-main button-small">查看业务情况</a>
      </template>
    </card-common>
  </div>
</template>
...
.color-main {
  ...
  border-color: #0a4b8b;
  border-top-color: #0a4b8b;
  border-right-color: #0a4b8b;
  border-bottom-color: #0a4b8b;
  border-left-color: #0a4b8b;
  color: #fff;
}
</style>